        ...
        try {
          ...
          await vscode.window.withProgress(
            ...
            async (progress) => {
              ...
              logger.info(
                `Extension.js path: ${extensionInfo.extensionJsPath}`,
              );
              ...
              const fixResult = findAndRemoveHeader(
                extensionInfo.extensionJsPath,
              );
              ...
              const verifyResult = verifyCopilotFix(
                extensionInfo.extensionJsPath,
              );
          ...
          logger.error("Error fixing Copilot Chat extension:", error);
          vscode.window.showErrorMessage(
            `Failed to fix Copilot Chat extension: ${(error as Error).message}`,
          );
        }
      },
    ),
